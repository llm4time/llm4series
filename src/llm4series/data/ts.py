...
class UniTimeSeries(
  ...
  def resid(self, strength: bool = False, period: int = None, freq: str = None, decimals: int = 4) -> Union['UniTimeSeries', float]:
    res = self.stl(period, freq, decimals)
    if strength:
...
class MultiTimeSeries(
  ...
  def resid(self, strength: bool = False, period: int = None, freq: str = None, decimals: int = 4) -> pd.DataFrame:
    res = self.stl(period, freq, decimals)
    if strength:
      return res["r_strength"]
    return res["resid"]
