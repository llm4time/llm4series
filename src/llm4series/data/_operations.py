...
class _UniTimeSeriesStatistics(TimeSeriesStatistics):
  ...
  def stl(self, period: int = None, freq: str = None, decimals: int = 4) -> pd.Series | None:
    ...
    if freq:
      ...
      try:
        ...
        return pd.Series({"trend": self.__class__(dtype=float), "seasonal": self.__class__(dtype=float),
                         "resid": self.__class__(dtype=float), "t_strength": pd.NA, "s_strength": pd.NA, "r_strength": pd.NA})
    ...
    try:
      ...
      T_var = np.var(T + R)
      S_var = np.var(S + R)
      total_var = np.var(T) + np.var(S) + np.var(R)
      t_strength = round(1 - np.var(R) / T_var, decimals) if T_var > 0 else np.nan
      s_strength = round(1 - np.var(R) / S_var, decimals) if S_var > 0 else np.nan
      r_strength = round(np.var(R) / total_var, decimals) if total_var > 0 else np.nan
      return pd.Series({"trend": self.__class__(T.round(decimals)), "seasonal": self.__class__(S.round(decimals)),
                       "resid": self.__class__(R.round(decimals)), "t_strength": t_strength,
  ...
  def stl(self, period: int = None, freq: str = None, decimals: int = 4) -> dict | None:
    ...
    for col in self.columns:
      ...
      if hasattr(ts, "stl"):
        ...
        if res is not None:
          trend_dict[col], seasonal_dict[col], resid_dict[col] = res["trend"], res["seasonal"], res["resid"]
    ...
    return {"trend": pd.DataFrame(trend_dict), "seasonal": pd.DataFrame(seasonal_dict),
            "resid": pd.DataFrame(resid_dict), "t_strength": pd.Series(t_strengths),
...
class _UniTimeSeriesPlot(TimeSeriesPlot):
  ...
  def stlplot(self, title: str = None, xlabel: str = None, ylabel: str = None,
    ...
    x = x or ["Trend", "Seasonal", "Resid"]
    fig = make_subplots(rows=3, cols=1, shared_xaxes=True, subplot_titles=x)
    colors = {"trend": "#FFA500", "seasonal": "#008000", "resid": "#FF0000"}
